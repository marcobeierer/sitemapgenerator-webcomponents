...
$jscomp.iteratorFromArray=function(b,a){$jscomp.initSymbolIterator();b instanceof String&&(b+="");var e=0,c={next:function(){if(e<b.length){var d=e++;return{value:a(d,b[d]),done:!1}}c.next=function(){return{done:!0,value:void 0}};return c.next()}};c[Symbol.iterator]=function(){return c};return c};
...
$jscomp.polyfill("String.prototype.startsWith",function(b){return b?b:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var d=c.length,h=a.length;b=Math.max(0,Math.min(b|0,c.length));for(var e=0;e<h&&b<d;)if(c[b++]!=a[e++])return!1;return e>=h}},"es6","es3");
riot.tag2("message",'<div if="{text != \'\' && !dismissed}" class="alert alert-{type} {alert-dismissible: dismissible}" riot-style="margin-top:; 15px; {style}"> <button if="{dismissible}" type="button" class="close" aria-label="Close"><span aria-hidden="true" onclick="{dismiss}">&times;</span></button> <raw content="{text}"></raw> </div>',"","",function(b){var a=this;a.plugin=b.plugin||console.error("no plugin set");a.name=b.name;a.text=b.text||"";a.type=b.type||"";a.dismissible=b.dismissible||!1;
a.style=b.style||"";a.dismissed=!1;a.plugin.on("set-message",function(b,c,d){void 0!=d&&d!=a.name||void 0==d&&void 0!=a.name||(a.text=b,a.type=c,a.dismissed=!1,a.update())});a.dismiss=function(b){a.dismissed=!0}});riot.tag2("raw","<span></span>","","",function(b){var a=this;a.root.innerHTML=b.content;a.on("update",function(){a.root.innerHTML=b.content})});riot.tag2("sitemap-generator",'<form name="sitemapForm" onsubmit="{generate}" style="margin-bottom: 20px;"> <div class="form-group" if="{showWebsiteURLInput()}"> <label>{tr(\'WEBSITE_URL_LABEL\')}</label> <input type="url" class="form-control" name="websiteUrl" placeholder="https://example.com" riot-value="{enteredWebsiteURL}" oninput="{setWebsiteURL}" disabled="{generateDisabled}" required> </div> <div class="form-group" if="{showWebsiteURLInput()}"> <label>{tr(\'TOKEN_LABEL\')}</label> <textarea class="form-control" name="token" rows="4" placeholder="{tr(\'TOKEN_DESC\')}" riot-value="{enteredToken}" oninput="{setToken}" disabled="{generateDisabled}"></textarea> </div> <div class="btn-group" role="group"> <button type="submit" class="btn {generateClass}" if="{!generateDisabled}">{tr(\'GENERATE_SITEMAP_BUTTON\')}</button> <button type="button" class="btn btn-danger" onclick="{stopGeneration}" if="{generateDisabled}">{tr(\'STOP_GENERATION_BUTTON\')}</button> <a class="btn {downloadClass}" onclick="{download}" disabled="{downloadDisabled}" download="sitemap.xml" href="{href}">{tr(\'DOWNLOAD_BUTTON\')}</a> </div> </form> <message plugin="{events}" text="{tr(\'INIT_MESSAGE\')}" type="warning"></message> <ul if="{showTabs()}" class="nav nav-tabs" id="tabnav{id}" role="tablist"> <li role="presentation" class="{active: isActiveTab(\'stats\')}"><a href="#stats{id}" aria-controls="stats{id}" role="tab" data-toggle="tab" onclick="{showStatsTab}">{tr(\'STATS_TAB\')}</a></li> <li role="presentation" class="{active: isActiveTab(\'config\')}"><a href="#config{id}" aria-controls="config{id}" role="tab" data-toggle="tab" onclick="{showConfigTab}">{tr(\'CONFIG_TAB\')}</a></li> </ul> <div if="{showTabs()}" class="tab-content"> <div role="tabpanel" class="tab-pane {active: isActiveTab(\'stats\')}" id="stats{id}"> <p if="{!stats}" style="margin-top: 15px;">{tr(\'NO_STATS_MESSAGE\')}</p> <div class="row"> <div if="{stats}" class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">{tr(\'SITEMAP_STATS_HEADING\')} <span title="{tr(\'STATS_FINISHED_AT_TITLE\')} {datetime(stats.FinishedAt)}" class="pull-right">{datetime(stats.FinishedAt)}</span></div> <table class="table table-bordered"> <tr> <td>{tr(\'SITEMAP_URL_COUNT\')}</td> <td class="text-right" style="width: 200px;">{stats.SitemapURLCount}</td> </tr> <tr if="{hasToken()}"> <td>{tr(\'SITEMAP_IMAGE_COUNT\')}</td> <td class="text-right">{stats.SitemapImageCount}</td> </tr> <tr if="{hasToken()}"> <td>{tr(\'SITEMAP_VIDEO_COUNT\')}</td> <td class="text-right">{stats.SitemapVideoCount}</td> </tr> </table> </div> </div> <div if="{stats && hasToken()}" class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">{tr(\'DOWNLOAD_LAST_SITEMAP_HEADING\')} <span title="{tr(\'DOWNLOADS_FINISHED_AT_TITLE\')} {datetime(stats.FinishedAt)}" class="pull-right">{datetime(stats.FinishedAt)}</span></div> <table class="table table-bordered"> <tr> <td><a href="{apiserverURL(\'/sitemap.xml?download=1\');}">sitemap.xml</a></td> </tr> <tr each="{subSitemap in subSitemaps()}"> <td><a href="{apiserverURL(\'/\' + subSitemap + \'?download=1\')}">{subSitemap}</a></td> </tr> </table> </div> </div> <div if="{stats}" class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">{tr(\'CRAWL_STATS_HEADING\')} <span title="{tr(\'STATS_FINISHED_AT_TITLE\')} {datetime(stats.FinishedAt)}" class="pull-right">{datetime(stats.FinishedAt)}</span></div> <table class="table table-bordered"> <tr> <td>{tr(\'CRAWLED_URLS_COUNT\')}</td> <td class="text-right" style="width: 200px;">{stats.CrawledResourcesCount}</td> </tr> <tr> <td>{tr(\'DEAD_URLS_COUNT\')}</td> <td class="text-right">{stats.DeadResourcesCount}</td> </tr> <tr> <td>{tr(\'TIMED_OUT_URLS_COUNT\')}</td> <td class="text-right">{stats.TimedOutResourcesCount}</td> </tr> <tr> <td>{tr(\'STARTED_AT\')}</td> <td class="text-right">{datetime(stats.StartedAt)}</td> </tr> <tr> <td>{tr(\'FINISHED_AT\')}</td> <td class="text-right">{datetime(stats.FinishedAt)}</td> </tr> <tr> <td colspan="2"> <i><raw content="{tr(\'LINK_CHECKER_PROMO\')}"></raw></i> </td> </tr> </table> </div> </div> <div if="{stats}" class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">{tr(\'SETTING_STATS_HEADING\')} <span title="{tr(\'STATS_FINISHED_AT_TITLE\')} {datetime(stats.FinishedAt)}" class="pull-right">{datetime(stats.FinishedAt)}</span></div> <table class="table table-bordered"> <tr> <td>{tr(\'CRAWL_DELAY\')}</td> <td class="text-right" style="width: 200px;">{stats.CrawlDelayInSeconds} {tr(\'SECONDS_UNIT\')}</td> </tr> <tr> <td>{tr(\'CONCURRENT_FETCHERS\')}</td> <td class="text-right">{stats.MaxFetchers}</td> </tr> <tr> <td>{tr(\'URL_LIMIT\')}</td> <td class="text-right">{stats.URLLimit} {tr(\'URLS_UNIT\')}</td> </tr> <tr> <td>{tr(\'LIMIT_REACHED\')}</td> <td class="text-right">{bool2text(stats.URLLimitReached)}</td> </tr> <tr> <td>{tr(\'TOKEN_USED\')}</td> <td class="text-right">{bool2text(stats.TokenUsed)}</td> </tr> </table> </div> </div> </div> </div> <div role="tabpanel" class="tab-pane {active: isActiveTab(\'config\')}" id="config{id}"> <div class="help-block config-help" style="margin-top: 15px;"><raw content="{tr(\'CONFIG_DESC\')}"></raw></div> <div class="form-group"> <label>{tr(\'IGNORE_EMBEDDED_CONTENT_LABEL\')}</label> <select class="form-control" name="ignoreEmbeddedContent" riot-value="{configIgnoreEmbeddedContent}" onchange="{setIgnoreEmbeddedContent}" disabled="{generateDisabled}"> <option value="0">{tr(\'NO\')}</option> <option value="1">{tr(\'YES\')}</option> </select> <div class="help-block config-help"><raw content="{tr(\'IGNORE_EMBEDDED_CONTENT_DESC\')}"></raw></div> </div> <div class="form-group"> <label>{tr(\'MAX_FETCHERS_LABEL\')}</label> <input class="form-control" name="maxFetchers" min="1" max="10" step="1" riot-value="{configMaxFetchers}" oninput="{setMaxFetchers}" disabled="{generateDisabled}" type="{\'number\'}"> <div class="help-block config-help"><raw content="{tr(\'MAX_FETCHERS_DESC\')}"></raw></div> </div> <div class="form-group"> <label>{tr(\'REFERENCE_COUNT_THRESHOLD_LABEL\')}</label> <select class="form-control" name="referenceCountThreshold" riot-value="{configReferenceCountThreshold}" onchange="{setReferenceCountThreshold}" disabled="{generateDisabled}"> <option each="{option in referenceCountThresholdOptions}" riot-value="{option.value}">{option.label}</option> </select> <div class="help-block config-help"><raw content="{tr(\'REFERENCE_COUNT_THRESHOLD_DESC\')}"></raw></div> </div> <div class="form-group"> <label>{tr(\'QUERY_PARAMS_TO_REMOVE_LABEL\')}</label> <input type="text" class="form-control" name="queryParamsToRemove" riot-value="{configQueryParamsToRemove}" oninput="{setQueryParamsToRemove}" disabled="{generateDisabled}"> <div class="help-block config-help"><raw content="{tr(\'QUERY_PARAMS_TO_REMOVE_DESC\')}"></raw></div> </div> <div class="form-group"> <label>{tr(\'DISABLE_COOKIES_LABEL\')}</label> <select class="form-control" name="disableCookies" riot-value="{configDisableCookies}" onchange="{setDisableCookies}" disabled="{generateDisabled}"> <option value="0">{tr(\'NO\')}</option> <option value="1">{tr(\'YES\')}</option> </select> <div class="help-block config-help"><raw content="{tr(\'DISABLE_COOKIES_DESC\')}"></raw></div> </div> </div> </div>',
'sitemap-generator .config-help,[data-is="sitemap-generator"] .config-help,sitemap-generator .config-help p,[data-is="sitemap-generator"] .config-help p,sitemap-generator .config-help pre,[data-is="sitemap-generator"] .config-help pre{ font-size: 14px; line-height: 1.6; } sitemap-generator .config-help p,[data-is="sitemap-generator"] .config-help p{ margin: 0 0 5px; } sitemap-generator .config-help pre,[data-is="sitemap-generator"] .config-help pre{ margin: 5px 0 0; } sitemap-generator .config-help p:last-child,[data-is="sitemap-generator"] .config-help p:last-child{ margin-bottom: 0; }',
"",function(b){var a=this;a.translations={en:{NO:"No",YES:"Yes",GENERATE_SITEMAP_BUTTON:"Generate sitemap",STOP_GENERATION_BUTTON:"Stop generation",DOWNLOAD_BUTTON:"Download",INIT_MESSAGE:"Sitemap Generator is initializing, please wait a moment.",STATS_TAB:"Stats",CONFIG_TAB:"Config",NO_STATS_MESSAGE:"No stats available yet. Generate a sitemap to see stats here.",SITEMAP_STATS_HEADING:"Sitemap Stats",STATS_FINISHED_AT_TITLE:"Stats for generation process that has finished at",DOWNLOADS_FINISHED_AT_TITLE:"Downloads for generation process that has finished at",
SITEMAP_URL_COUNT:"Sitemap URL count",SITEMAP_IMAGE_COUNT:"Sitemap image count",SITEMAP_VIDEO_COUNT:"Sitemap video count",DOWNLOAD_LAST_SITEMAP_HEADING:"Download Last Sitemap (or Index File and Sitemaps)",CRAWL_STATS_HEADING:"Crawl Stats",CRAWLED_URLS_COUNT:"Crawled URLs count",DEAD_URLS_COUNT:"Dead URLs count",TIMED_OUT_URLS_COUNT:"Timed out URLs count",STARTED_AT:"Started at",FINISHED_AT:"Finished at",LINK_CHECKER_PROMO:'Want find out more about the dead and timed out URLs? Have a look at my <a href="https://www.marcobeierer.com/tools/link-checker" target="_blank">Link Checker</a>.',
SETTING_STATS_HEADING:"Setting Stats",CRAWL_DELAY:"Crawl delay",SECONDS_UNIT:"seconds",CONCURRENT_FETCHERS:"Concurrent fetchers",URL_LIMIT:"URL limit",URLS_UNIT:"URLs",LIMIT_REACHED:"Limit reached",TOKEN_USED:"Token used",CONFIG_DESC:"Use the Sitemap Generator to create a sitemap with up to 500 URLs for free. For more URLs or image and video sitemaps, buy a token for <a href='https://www.marcobeierer.com/tools/sitemap-generator/pro'>Sitemap Generator Pro</a>. It lets you create sitemaps with up to 50,000 URLs.",
WEBSITE_URL_LABEL:"Website URL",TOKEN_LABEL:"Token",TOKEN_DESC:"Optional token for Sitemap Generator Pro. Without a token, the Sitemap Generator works for websites with up to 500 URLs.",IGNORE_EMBEDDED_CONTENT_LABEL:"Ignore Embedded Content",IGNORE_EMBEDDED_CONTENT_DESC:"Does not add embedded content such as images and videos to the sitemap. This option is only useful with a valid token. Without a token, embedded content is never added to the sitemap.",MAX_FETCHERS_LABEL:"Concurrent Connections",
MAX_FETCHERS_DESC:"Maximum number of concurrent connections. The default is three. Some hosts allow fewer connections. Limit the connections here so the Sitemap Generator still works. Increase the value only if your server can handle it.",REFERENCE_COUNT_THRESHOLD_LABEL:"Reference Count Threshold",REFERENCE_COUNT_THRESHOLD_DESC:"<p>With the reference count threshold, exclude images and videos embedded on more than the selected number of HTML pages.</p><p>This is useful for header or footer images that should not appear in the sitemap.</p><p>Without a threshold, all images and videos are included, even if they are embedded on all HTML pages. At 0, no images and videos are included. At 5, all images and videos embedded on more than 5 HTML pages are excluded. If an excluded image or video is also embedded on the front page, it is assigned to the front page and included once in the sitemap. Linked, non-embedded images are always added.</p><p>The value does not affect the URL limit because images and videos are still crawled.</p>",
QUERY_PARAMS_TO_REMOVE_LABEL:"Query Params To Remove",QUERY_PARAMS_TO_REMOVE_DESC:"<p>List of query parameters separated by ampersands (&amp;). These parameters are removed from each URL before processing, for example timestamps.</p><p>For <em>q</em> and <em>timestamp</em>, this value is valid:</p><pre>q&amp;timestamp</pre>",DISABLE_COOKIES_LABEL:"Disable Cookies",DISABLE_COOKIES_DESC:"<p>The Sitemap Generator crawler uses cookies by default. Cookies set by your website are used for subsequent requests and deleted after each run.</p><p>Cookie support should usually stay enabled because visitors usually have cookies enabled and the Sitemap Generator can behave like a visitor. Disable cookies only if needed.</p>",
NO_THRESHOLD:"No Threshold",SITEMAP_GENERATOR_NOT_STARTED:"The Sitemap Generator was not started yet.",GENERATION_NOT_STARTED:"The generation of the sitemap was not started yet.",ENTER_WEBSITE_URL:"Please enter a website URL before generating the sitemap.",GENERATION_IN_PROGRESS:"The sitemap is being generated. Please wait a moment.",URLS_PROCESSED:"{count} URLs already processed.",URL_LIMIT_REACHED_MESSAGE:'The Sitemap Generator reached the URL limit and the generated sitemap probably is not complete. You may buy a token for the <a href="{professionalUrl}">Sitemap Generator Professional</a> to crawl up to 50\'000 URLs and create a complete sitemap. Additionally to a higher URL limit, the professional version also adds images and videos to your sitemap.',
GENERATION_SUCCESS_DOWNLOAD:"Your sitemap was generated successfully. You can download the sitemap now.",GENERATION_SUCCESS_SAVED:"The generation of the sitemap was successful. The sitemap was saved with the filename {filename} in the root folder of your {systemName} instance. Please see the stats below.",TOKEN_INVALID:"The validation of your token failed. The token is invalid or has expired. Please try it again or contact me if the token should be valid.",WRITE_ERROR:"Could not write sitemap to file. The reason for that could be a permission issue or that not enough space is available.",
GENERATION_FAILED_WITH_ERROR:"The generation of your sitemap failed with the error:<br/><strong>{error}</strong>.",GENERATION_FAILED_TRY_AGAIN:"The generation of your sitemap failed. Please try it again.",BACKEND_UNAVAILABLE:"The backend server is temporarily unavailable. Please try it again later.",CURL_ERROR_EMPTY:"A cURL error occurred. Please contact the developer of the extensions.",CURL_ERROR_WITH_MESSAGE:"A cURL error occurred with the error message:<br/><strong>{message}</strong>.",GENERATION_FAILED_CONTACT:"The generation of your sitemap failed. Please try it again or contact the developer of the extensions.",
STOPPING_GENERATION:"Going to stop the current generation.",GENERATION_STOPPED:"The current generation was stopped successfully.",STOP_FAILED:"Could not stop the generation because the connection to the server failed."},de:{NO:"Nein",YES:"Ja",GENERATE_SITEMAP_BUTTON:"Sitemap generieren",STOP_GENERATION_BUTTON:"Generierung stoppen",DOWNLOAD_BUTTON:"Herunterladen",INIT_MESSAGE:"Sitemap-Generator wird initialisiert. Bitte einen Moment warten.",STATS_TAB:"Statistiken",CONFIG_TAB:"Konfiguration",NO_STATS_MESSAGE:"Noch keine Statistiken verf\u00fcgbar. Generiere eine Sitemap, um hier Statistiken zu sehen.",
SITEMAP_STATS_HEADING:"Sitemap-Statistiken",STATS_FINISHED_AT_TITLE:"Statistiken f\u00fcr den Generierungsvorgang, der abgeschlossen wurde am",DOWNLOADS_FINISHED_AT_TITLE:"Downloads f\u00fcr den Generierungsvorgang, der abgeschlossen wurde am",SITEMAP_URL_COUNT:"Anzahl der Sitemap-URLs",SITEMAP_IMAGE_COUNT:"Anzahl der Sitemap-Bilder",SITEMAP_VIDEO_COUNT:"Anzahl der Sitemap-Videos",DOWNLOAD_LAST_SITEMAP_HEADING:"Letzte Sitemap herunterladen (oder Indexdatei und Sitemaps)",CRAWL_STATS_HEADING:"Crawl-Statistiken",
CRAWLED_URLS_COUNT:"Anzahl gecrawlter URLs",DEAD_URLS_COUNT:"Anzahl defekter URLs",TIMED_OUT_URLS_COUNT:"Anzahl der URLs mit Zeit\u00fcberschreitung",STARTED_AT:"Gestartet am",FINISHED_AT:"Abgeschlossen am",LINK_CHECKER_PROMO:'M\u00f6chtest du mehr \u00fcber die defekten URLs und URLs mit Zeit\u00fcberschreitung erfahren? Sieh dir meinen <a href="https://www.marcobeierer.com/tools/link-checker" target="_blank">Link Checker</a> an.',SETTING_STATS_HEADING:"Einstellungen",CRAWL_DELAY:"Crawl-Verz\u00f6gerung",
SECONDS_UNIT:"Sekunden",CONCURRENT_FETCHERS:"Gleichzeitige Verbindungen",URL_LIMIT:"URL-Limit",URLS_UNIT:"URLs",LIMIT_REACHED:"Limit erreicht",TOKEN_USED:"Token verwendet",CONFIG_DESC:"Mit dem Sitemap-Generator kannst du kostenlos eine Sitemap mit bis zu 500 URLs erstellen. F\u00fcr mehr URLs oder Bild- und Video-Sitemaps kannst du einen Token f\u00fcr <a href='https://www.marcobeierer.com/tools/sitemap-generator/pro'>Sitemap-Generator Pro</a> kaufen. Damit erstellst du Sitemaps mit bis zu 50.000 URLs.",
WEBSITE_URL_LABEL:"Website-URL",TOKEN_LABEL:"Token",TOKEN_DESC:"Optionaler Token f\u00fcr Sitemap-Generator Pro. Ohne Token funktioniert der Sitemap-Generator f\u00fcr Websites mit bis zu 500 URLs.",IGNORE_EMBEDDED_CONTENT_LABEL:"Eingebettete Inhalte ignorieren",IGNORE_EMBEDDED_CONTENT_DESC:"F\u00fcgt eingebettete Inhalte wie Bilder und Videos nicht zur Sitemap hinzu. Diese Option ist nur mit g\u00fcltigem Token sinnvoll. Ohne Token werden eingebettete Inhalte nie zur Sitemap hinzugef\u00fcgt.",MAX_FETCHERS_LABEL:"Gleichzeitige Verbindungen",
MAX_FETCHERS_DESC:"Maximale Anzahl gleichzeitiger Verbindungen. Standard sind drei. Manche Hoster erlauben weniger Verbindungen. Begrenze hier die Verbindungen, damit der Sitemap-Generator trotzdem funktioniert. Erh\u00f6he den Wert nur, wenn dein Server das verkraftet.",REFERENCE_COUNT_THRESHOLD_LABEL:"Referenzanzahl-Schwellenwert",REFERENCE_COUNT_THRESHOLD_DESC:"<p>Mit dem Referenzanzahl-Schwellenwert schliesst du Bilder und Videos aus, die auf mehr als der gew\u00e4hlten Anzahl von HTML-Seiten eingebettet sind.</p><p>Das ist n\u00fctzlich f\u00fcr Header- oder Footer-Bilder, die nicht in die Sitemap sollen.</p><p>Ohne Schwellenwert werden alle Bilder und Videos aufgenommen, auch wenn sie auf allen HTML-Seiten eingebettet sind. Bei 0 werden keine Bilder und Videos aufgenommen. Bei 5 werden alle ausgeschlossen, die auf mehr als 5 HTML-Seiten eingebettet sind. Ist ein ausgeschlossenes Bild oder Video auch auf der Startseite eingebettet, wird es der Startseite zugeordnet und einmal in die Sitemap aufgenommen. Verlinkte, nicht eingebettete Bilder werden immer hinzugef\u00fcgt.</p><p>Der Wert beeinflusst das URL-Limit nicht, weil Bilder und Videos trotzdem gecrawlt werden.</p>",
QUERY_PARAMS_TO_REMOVE_LABEL:"Zu entfernende Query-Parameter",QUERY_PARAMS_TO_REMOVE_DESC:"<p>Liste von Query-Parametern, getrennt durch kaufm\u00e4nnische Und-Zeichen (&amp;). Diese Parameter werden vor der Verarbeitung aus jeder URL entfernt, etwa Zeitstempel.</p><p>F\u00fcr <em>q</em> und <em>timestamp</em> ist dieser Wert g\u00fcltig:</p><pre>q&amp;timestamp</pre>",DISABLE_COOKIES_LABEL:"Cookies deaktivieren",DISABLE_COOKIES_DESC:"<p>Der Sitemap-Generator-Crawler nutzt Cookies standardm\u00e4ssig. Von deiner Website gesetzte Cookies werden f\u00fcr folgende Requests verwendet und nach jedem Lauf gel\u00f6scht.</p><p>Cookie-Unterst\u00fctzung sollte meist aktiv bleiben, weil Besucher Cookies \u00fcblicherweise aktiviert haben und der Sitemap-Generator sich so wie ein Besucher verh\u00e4lt. Deaktiviere Cookies nur bei Bedarf.</p>",
NO_THRESHOLD:"Kein Schwellenwert",SITEMAP_GENERATOR_NOT_STARTED:"Der Sitemap-Generator wurde noch nicht gestartet.",GENERATION_NOT_STARTED:"Die Generierung der Sitemap wurde noch nicht gestartet.",ENTER_WEBSITE_URL:"Bitte gib eine Website-URL ein, bevor du die Sitemap generierst.",GENERATION_IN_PROGRESS:"Die Sitemap wird generiert. Bitte einen Moment warten.",URLS_PROCESSED:"{count} URLs bereits verarbeitet.",URL_LIMIT_REACHED_MESSAGE:'Der Sitemap-Generator hat das URL-Limit erreicht. Die generierte Sitemap ist wahrscheinlich unvollst\u00e4ndig. Mit einem Token f\u00fcr den <a href="{professionalUrl}">Sitemap-Generator Professional</a> kannst du bis zu 50.000 URLs crawlen und eine vollst\u00e4ndige Sitemap erstellen. Die Professional-Version f\u00fcgt der Sitemap ausserdem Bilder und Videos hinzu.',
GENERATION_SUCCESS_DOWNLOAD:"Deine Sitemap wurde erfolgreich generiert. Du kannst die Sitemap jetzt herunterladen.",GENERATION_SUCCESS_SAVED:"Die Generierung der Sitemap war erfolgreich. Die Sitemap wurde mit dem Dateinamen {filename} im Root-Verzeichnis deiner {systemName}-Instanz gespeichert. Bitte siehe die Statistiken unten.",TOKEN_INVALID:"Die \u00dcberpr\u00fcfung deines Tokens ist fehlgeschlagen. Der Token ist ung\u00fcltig oder abgelaufen. Bitte versuche es erneut oder kontaktiere mich, falls der Token g\u00fcltig sein sollte.",
WRITE_ERROR:"Die Sitemap konnte nicht in die Datei geschrieben werden. Grund daf\u00fcr k\u00f6nnte ein Berechtigungsproblem sein oder dass nicht gen\u00fcgend Speicherplatz verf\u00fcgbar ist.",GENERATION_FAILED_WITH_ERROR:"Die Generierung deiner Sitemap ist mit folgendem Fehler fehlgeschlagen:<br/><strong>{error}</strong>.",GENERATION_FAILED_TRY_AGAIN:"Die Generierung deiner Sitemap ist fehlgeschlagen. Bitte versuche es erneut.",BACKEND_UNAVAILABLE:"Der Backend-Server ist vor\u00fcbergehend nicht verf\u00fcgbar. Bitte versuche es sp\u00e4ter erneut.",
CURL_ERROR_EMPTY:"Ein cURL-Fehler ist aufgetreten. Bitte kontaktiere den Entwickler der Erweiterungen.",CURL_ERROR_WITH_MESSAGE:"Ein cURL-Fehler ist mit folgender Fehlermeldung aufgetreten:<br/><strong>{message}</strong>.",GENERATION_FAILED_CONTACT:"Die Generierung deiner Sitemap ist fehlgeschlagen. Bitte versuche es erneut oder kontaktiere den Entwickler der Erweiterungen.",STOPPING_GENERATION:"Die aktuelle Generierung wird gestoppt.",GENERATION_STOPPED:"Die aktuelle Generierung wurde erfolgreich gestoppt.",
STOP_FAILED:"Die Generierung konnte nicht gestoppt werden, weil die Verbindung zum Server fehlgeschlagen ist."}};a.language="de"==b.language?"de":"en";a.tr=function(b){return(a.translations[a.language]||a.translations.en)[b]||a.translations.en[b]||b};a.trFormat=function(b,d){var c=a.tr(b);d=d||{};Object.keys(d).forEach(function(a){c=c.replace(new RegExp("\\{"+a+"\\}","g"),d[a])});return c};a.optionValue=function(a,b){return[void 0,null,""].includes(a)?b:a};a.referenceCountThresholdOptions=[{value:"-1",
label:a.tr("NO_THRESHOLD")}];for(var e=1;100>=e;e++)a.referenceCountThresholdOptions.push({value:String(e),label:String(e)});a.events=riot.observable();a.downloadDisabled=!0;a.generateDisabled=!1;a.pageCount=0;a.stats=null;a.forceStop=!1;a.sitemapGeneratorBlob;a.href;a.enteredWebsiteURL="";a.enteredToken=b.token||"";a.configIgnoreEmbeddedContent=String(a.optionValue(b.ignoreEmbeddedContent,"0"));a.configMaxFetchers=String(a.optionValue(b.maxFetchers,3));a.configReferenceCountThreshold=String(a.optionValue(b.referenceCountThreshold,
5));a.configQueryParamsToRemove=a.optionValue(b.queryParamsToRemove,"");a.configDisableCookies=String(a.optionValue(b.disableCookies,"0"));a.id=b.id||a._riot_id||0;a.showWebsiteURLInput=function(){return""==(b.websiteUrl||"").trim()};a.showTabs=function(){return[!0,"true","1",1].includes(b.showTabs)};a.activeTab=a.showWebsiteURLInput()?"config":"stats";a.isActiveTab=function(b){return a.activeTab==b};a.showTab=function(b,d){d&&d.preventDefault();a.activeTab=b};a.showStatsTab=function(b){a.showTab("stats",
b)};a.showConfigTab=function(b){a.showTab("config",b)};a.setWebsiteURL=function(b){a.enteredWebsiteURL=b.target.value};a.setToken=function(b){a.enteredToken=b.target.value};a.setIgnoreEmbeddedContent=function(b){a.configIgnoreEmbeddedContent=b.target.value};a.setMaxFetchers=function(b){a.configMaxFetchers=b.target.value};a.setReferenceCountThreshold=function(b){a.configReferenceCountThreshold=b.target.value};a.setQueryParamsToRemove=function(b){a.configQueryParamsToRemove=b.target.value};a.setDisableCookies=
function(b){a.configDisableCookies=b.target.value};a.websiteURL=function(){return(b.websiteUrl||a.enteredWebsiteURL||"").trim()};a.websiteURL64=function(){var b=window.btoa(encodeURIComponent(a.websiteURL()).replace(/%([0-9A-F]{2})/g,function(a,b){return String.fromCharCode("0x"+b)}));b.replace(/\+/g,"-").replace(/\//g,"_");return b};a.token=function(){var c=a.showWebsiteURLInput()?a.enteredToken:b.token;return c?c.replace(/\s/g,""):c};a.hasToken=function(){return![void 0,null,""].includes(a.token())};
a.identifier=function(){return b.identifier||""};a.systemName=function(){return b.systemName||"Unknown"};a.btnPrimaryClass=function(){return b.btnPrimaryClass||"btn-primary"};a.btnDefaultClass=function(){return b.btnDefaultClass||"btn-default"};a.maxFetchers=function(){return a.configMaxFetchers||3};a.ignoreEmbeddedContent=function(){return a.configIgnoreEmbeddedContent};a.referenceCountThreshold=function(){return a.configReferenceCountThreshold||5};a.queryParamsToRemove=function(){return a.configQueryParamsToRemove||
""};a.disableCookies=function(){return a.configDisableCookies};a.enableIndexFile=function(){return b.enableIndexFile||""};a.sitemapFilename=function(){return b.sitemapFilename||"sitemap.xml"};a.setMessage=function(b,d,e){a.events.trigger("set-message",b,d,e)};a.on("before-mount",function(){a.generateClass=a.btnPrimaryClass();a.downloadClass=a.btnDefaultClass()});a.on("mount",function(){if(""!=a.websiteURL()){var b="";a.hasToken()&&(b="BEARER "+a.token());a.websiteURL64();var d=a.apiserverURL("/running");
jQuery.ajax({method:"GET",url:d,headers:{Authorization:b,"Cache-Control":"no-store"}}).done(function(b,c,d){b.Running?a.generate():a.setMessage(a.tr("SITEMAP_GENERATOR_NOT_STARTED"),"info");b.SitemapAvailable&&a.loadDataFromServer()}).fail(function(b){a.setMessage(a.tr("SITEMAP_GENERATOR_NOT_STARTED"),"info")})}else setTimeout(function(){a.setMessage(a.tr("GENERATION_NOT_STARTED"),"info")},500)});a.apiserverURL=function(c){void 0==c&&(c="");var d=b.proxyUrl;d&&""==c||(d=b.dev?"http://marco-desktop:9999/sitemap/v2/"+
a.websiteURL64():"https://api.marcobeierer.com/sitemap/v2/"+a.websiteURL64());""==c&&(c="",c=b.proxyUrl?c+"&":c+"?",c=c+"pdfs=1&origin_system="+encodeURIComponent(a.systemName().toLowerCase()),c+="&max_fetchers="+encodeURIComponent(a.maxFetchers()),c+="&ignore_embedded_content="+encodeURIComponent(a.ignoreEmbeddedContent()),c+="&reference_count_threshold="+encodeURIComponent(a.referenceCountThreshold()),c+="&query_params_to_remove="+encodeURIComponent(a.queryParamsToRemove()),c+="&disable_cookies="+
encodeURIComponent(a.disableCookies()),c+="&enable_index_file="+encodeURIComponent(a.enableIndexFile()),b.proxyUrl&&(c+="&baseurl64="+a.websiteURL64(),""!=a.identifier()&&(c+="&identifier="+a.identifier())));return d+c};a.generate=function(c){c&&c.preventDefault();if(!a.generateDisabled)if(""==a.websiteURL())a.setMessage(a.tr("ENTER_WEBSITE_URL"),"danger");else{a.downloadDisabled=!0;a.generateDisabled=!0;a.pageCount=0;var d=a.tr("GENERATION_IN_PROGRESS");a.setMessage(d,"warning");a.generateClass=
a.btnPrimaryClass();a.downloadClass=a.btnDefaultClass();a.retries=0;var e=function(){var c="";a.hasToken()&&(c="BEARER "+a.token());jQuery.ajax({method:"GET",url:a.apiserverURL(),headers:{Authorization:c,"Cache-Control":"no-store"}}).done(function(f,h,g){a.retries=0;g.getResponseHeader("Content-Type").startsWith("application/xml")||g.getResponseHeader("Content-Type").startsWith("text/xml")?(a.sitemapGeneratorBlob=new Blob([g.responseText],{type:"application/xml"}),a.href=(window.URL||window.webkitURL).createObjectURL(a.sitemapGeneratorBlob),
a.downloadDisabled=!1,a.events.trigger("stopped"),null!=g.getResponseHeader("X-Stats")&&(a.stats=JSON.parse(g.getResponseHeader("X-Stats"))),a.stats&&a.stats.URLLimitReached?a.setMessage(a.trFormat("URL_LIMIT_REACHED_MESSAGE",{professionalUrl:b.professionalUrl||""}),"danger"):(f=a.tr("GENERATION_SUCCESS_DOWNLOAD"),["WordPress","Joomla"].includes(a.systemName())&&(f=a.trFormat("GENERATION_SUCCESS_SAVED",{filename:a.sitemapFilename(),systemName:a.systemName()})),a.setMessage(f,"success")),a.generateClass=
a.btnDefaultClass(),a.downloadClass=a.btnPrimaryClass()):(a.pageCount=f.page_count,0<a.pageCount&&(f=d+"<br />"+a.trFormat("URLS_PROCESSED",{count:a.pageCount}),a.setMessage(f,"warning")),a.forceStop?a.stop(a.apiserverURL(),c):setTimeout(e,1E3));a.update()}).fail(function(b,d,h){d=b.status;a.events.trigger("stopped");if(401==d)a.setMessage(a.tr("TOKEN_INVALID"),"danger");else if(500==d)1==b.getResponseHeader("X-Write-Error")?a.setMessage(a.tr("WRITE_ERROR"),"danger"):![void 0,null,""].includes(b.responseJSON)&&
b.getResponseHeader("Content-Type").startsWith("application/json")?a.setMessage(a.trFormat("GENERATION_FAILED_WITH_ERROR",{error:b.responseJSON}),"danger"):a.setMessage(a.tr("GENERATION_FAILED_TRY_AGAIN"),"danger");else if(503==d)a.setMessage(a.tr("BACKEND_UNAVAILABLE"));else if(504==d&&1==b.getResponseHeader("X-CURL-Error"))b=b.responseJSON,""==b?a.setMessage(a.tr("CURL_ERROR_EMPTY"),"danger"):a.setMessage(a.trFormat("CURL_ERROR_WITH_MESSAGE",{message:b}),"danger");else{if(0==d&&3>a.retries){a.retries++;
a.forceStop?a.stop(a.apiserverURL(),c):setTimeout(e,1E3);a.update();return}a.setMessage(a.tr("GENERATION_FAILED_CONTACT"),"danger")}a.update()})};e()}};a.download=function(b){window.navigator.msSaveOrOpenBlob&&a.sitemapGeneratorBlob&&window.navigator.msSaveOrOpenBlob(a.sitemapGeneratorBlob,"sitemap.xml")};a.stopGeneration=function(b){b.preventDefault();a.forceStop=!0};a.events.on("stopped",function(){a.generateDisabled=!1;a.update()});a.stop=function(b,d){a.setMessage(a.tr("STOPPING_GENERATION"),
"warning");a.update();jQuery.ajax({method:"DELETE",url:b,headers:{Authorization:d}}).done(function(b){a.setMessage(a.tr("GENERATION_STOPPED"),"info")}).fail(function(b){a.setMessage(a.tr("STOP_FAILED"),"danger")}).always(function(){a.forceStop=!1;a.events.trigger("stopped");a.update()})};a.loadDataFromServer=function(){if(a.hasToken()){var b="BEARER "+a.token();a.websiteURL64();var d=a.apiserverURL("/stats");jQuery.ajax({method:"GET",url:d,headers:{Authorization:b}}).done(function(b){a.stats=b}).fail(function(a){console.log("fetching saved result failed")}).always(function(){a.update()})}};
a.bool2text=function(b){return b?a.tr("YES"):a.tr("NO")};a.subSitemaps=function(){var b=[];if(!a.stats)return b;for(var d="",e=0;e<a.stats.SitemapIndexNumberOfDigits;e++)d+="0";for(e=0;e<a.stats.SitemapIndexCount;e++){var k=(d+e).slice(-a.stats.SitemapIndexNumberOfDigits);b.push("sitemap."+k+".xml")}return b};a.datetime=function(a){return(new Date(a)).toLocaleString()};a.datetimeAt=function(b){return a.datetime(b).replace(","," at")}});
