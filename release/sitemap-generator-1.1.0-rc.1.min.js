...
function(){return b.queryParamsToRemove||""};a.disableCookies=function(){return b.disableCookies||""};a.enableIndexFile=function(){return b.enableIndexFile||""};a.setMessage=function(c,b,d){a.events.trigger("set-message",c,b,d)};a.on("before-mount",function(){a.generateClass=a.btnPrimaryClass();a.downloadClass=a.btnDefaultClass()});a.on("mount",function(){if(""!=a.websiteURL()){var c="";a.hasToken()&&(c="BEARER "+a.token());a.websiteURL64();var b=a.apiserverURL("/running");jQuery.ajax({method:"GET",
url:b,headers:{Authorization:c,"Cache-Control":"no-store"}}).done(function(c,b,e){c.Running?a.generate():a.setMessage("The Sitemap Generator was not started yet.","info");c.SitemapAvailable&&a.loadDataFromServer()}).fail(function(c){a.setMessage("The Sitemap Generator was not started yet.","info")})}else setTimeout(function(){a.setMessage("The generation of the sitemap was not started yet.","info")},500)});a.apiserverURL=function(c){void 0==c&&(c="");if(!b.proxyURL||""!=c)return""==c&&(c="?pdfs=1&origin_system="+
encodeURIComponent(a.systemName().toLowerCase()),c+="&max_fetchers="+encodeURIComponent(a.maxFetchers()),c+="&ignore_embedded_content="+encodeURIComponent(a.ignoreEmbeddedContent()),c+="&reference_count_threshold="+encodeURIComponent(a.referenceCountThreshold()),c+="&query_params_to_remove="+encodeURIComponent(a.queryParamsToRemove()),c+="&disable_cookies="+encodeURIComponent(a.disableCookies()),c+="&enable_index_file="+encodeURIComponent(a.enableIndexFile())),b.dev?"http://marco-desktop:9999/sitemap/v2/"+
a.websiteURL64()+c:"https://api.marcobeierer.com/sitemap/v2/"+a.websiteURL64()+c;c=b.proxyURL;"Joomla"==a.systemName()&&(c+="&base64url="+a.websiteURL64()+"&identifier="+a.identifier());return c};a.generate=function(c){c&&c.preventDefault();a.downloadDisabled=!0;a.generateDisabled=!0;a.pageCount=0;a.setMessage("The sitemap is being generated. Please wait a moment.","warning");a.generateClass=a.btnPrimaryClass();a.downloadClass=a.btnDefaultClass();a.retries=0;var e=function(){var c="";a.hasToken()&&
(c="BEARER "+a.token());jQuery.ajax({method:"GET",url:a.apiserverURL(),headers:{Authorization:c,"Cache-Control":"no-store"}}).done(function(d,h,g){a.retries=0;g.getResponseHeader("Content-Type").startsWith("application/xml")||g.getResponseHeader("Content-Type").startsWith("text/xml")?(a.sitemapGeneratorBlob=new Blob([g.responseText],{type:"application/xml"}),a.href=(window.URL||window.webkitURL).createObjectURL(a.sitemapGeneratorBlob),a.downloadDisabled=!1,a.events.trigger("stopped"),1==g.getResponseHeader("X-Limit-Reached")?
a.setMessage("The Sitemap Generator reached the URL limit and the generated sitemap probably isn't complete. You may buy a token for the <a href=\""+b.professionalURL+"\">Sitemap Generator Professional</a> to crawl up to 50'000 URLs and create a complete sitemap. Additionally to a higher URL limit, the professional version also adds images and videos to your sitemap.","danger"):(d="Your sitemap was generated successfully. You can download the sitemap now.",["WordPress","Joomla"].includes(a.systemName())&&
(d="The generation of the sitemap was successful. The sitemap was saved as sitemap.xml in the "+a.systemName()+" root folder. Please see the stats below."),a.setMessage(d,"success")),null!=g.getResponseHeader("X-Stats")&&(a.stats=JSON.parse(g.getResponseHeader("X-Stats"))),a.generateClass=a.btnDefaultClass(),a.downloadClass=a.btnPrimaryClass()):(a.pageCount=d.page_count,0<a.pageCount&&(d="The sitemap is being generated. Please wait a moment.<br />"+a.pageCount+" URLs already processed.",a.setMessage(d,
"warning")),a.forceStop?a.stop(a.apiserverURL(),c):setTimeout(e,1E3));a.update()}).fail(function(b,d,g){d=b.status;a.events.trigger("stopped");if(401==d)a.setMessage("The validation of your token failed. The token is invalid or has expired. Please try it again or contact me if the token should be valid.","danger");else if(500==d)1==b.getResponseHeader("X-Write-Error")?a.setMessage("Could not write sitemap to file. The reason for that could be a permission issue or that not enough space is available.",
