...
$jscomp.polyfill("Array.prototype.includes",function(b){return b?b:function(a,b){var c=this;c instanceof String&&(c=String(c));var d=c.length;b=b||0;for(0>b&&(b=Math.max(b+d,0));b<d;b++){var g=c[b];if(g===a||Object.is(g,a))return!0}return!1}},"es7","es3");
...
$jscomp.polyfill("String.prototype.startsWith",function(b){return b?b:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var d=c.length,g=a.length;b=Math.max(0,Math.min(b|0,c.length));for(var h=0;h<g&&b<d;)if(c[b++]!=a[h++])return!1;return h>=g}},"es6","es3");
...
a.dismissed=!1;a.plugin.on("set-message",function(b,e,d){void 0!=d&&d!=a.name||void 0==d&&void 0!=a.name||(a.text=b,a.type=e,a.dismissed=!1,a.update())});a.dismiss=function(b){a.dismissed=!0}});riot.tag2("raw","<span></span>","","",function(b){var a=this;a.root.innerHTML=b.content;a.on("update",function(){a.root.innerHTML=b.content})});riot.tag2("sitemap-generator",'<div id="sitemap-widget"> <form name="sitemapForm"> <div class="input-group"> <span class="input-group-btn"> <button type="submit" class="btn {generateClass}" onclick="{generate}" disabled="{generateDisabled}">Sitemap generieren</button> </span> <a class="btn {downloadClass}" onclick="{download}" disabled="{downloadDisabled}" download="sitemap.xml" href="{href}">Download</a> </div> </form> <message plugin="{events}" text="Sitemap Generator is initializing, please wait a moment." type="warning"></message> </div> <div class="row"> <div if="{stats}" class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">Sitemap Stats</div> <table class="table table-bordered"> <tr> <td>Sitemap URL count</td> <td class="text-right" style="width: 200px;">{stats.SitemapURLCount}</td> </tr> <tr if="{hasToken()}"> <td>Sitemap image count</td> <td class="text-right">{stats.SitemapImageCount}</td> </tr> <tr if="{hasToken()}"> <td>Sitemap video count</td> <td class="text-right">{stats.SitemapVideoCount}</td> </tr> </table> </div> </div> <div if="{stats}" class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">Crawl Stats</div> <table class="table table-bordered"> <tr> <td>Crawled URLs count</td> <td class="text-right" style="width: 200px;">{stats.CrawledResourcesCount}</td> </tr> <tr> <td>Dead URLs count</td> <td class="text-right">{stats.DeadResourcesCount}</td> </tr> <tr> <td>Timed out URLs count</td> <td class="text-right">{stats.TimedOutResourcesCount}</td> </tr> <tr> <td colspan="2"> <i>Want find out more about the dead and timed out URLs? Have a look at my <a href="https://www.marcobeierer.com/tools/link-checker" target="_blank">Link Checker</a>.</i> </td> </tr> </table> </div> </div> <div class="col-lg-12"> <div class="panel panel-default"> <div class="panel-heading">Settings</div> <table class="table table-bordered"> <tr> <td>Token activated</td> <td class="text-right" style="width: 200px;">{bool2text(hasToken())}</td> </tr> <tr> <td>Concurrent fetchers</td> <td class="text-right">{maxFetchers()}</td> </tr> </table> </div> </div> </div>',
"","",function(b){var a=this;a.events=riot.observable();a.downloadDisabled=!0;a.generateDisabled=!1;a.pageCount=0;a.stats=null;a.sitemapGeneratorBlob;a.href;a.websiteURL64=function(){var a=window.btoa(encodeURIComponent(b.websiteUrl).replace(/%([0-9A-F]{2})/g,function(a,b){return String.fromCharCode("0x"+b)}));a.replace(/\+/g,"-").replace(/\//g,"_");return a};a.token=function(){return b.token?b.token.replace(/\s/g,""):b.token};a.hasToken=function(){return![void 0,null,""].includes(a.token())};a.identifier=
function(){return b.identifier||""};a.systemName=function(){return b.systemName||"Unknown"};a.btnPrimaryClass=function(){return b.btnPrimaryClass||"btn-primary"};a.btnDefaultClass=function(){return b.btnDefaultClass||"btn-default"};a.maxFetchers=function(){return b.maxFetchers||3};a.ignoreEmbeddedContent=function(){return b.ignoreEmbeddedContent||""};a.referenceCountThreshold=function(){return b.referenceCountThreshold||5};a.queryParamsToRemove=function(){return b.queryParamsToRemove||""};a.disableCookies=
function(){return b.disableCookies||""};a.enableIndexFile=function(){return b.enableIndexFile||""};a.setMessage=function(b,e,d){a.events.trigger("set-message",b,e,d)};a.on("before-mount",function(){a.generateClass=a.btnPrimaryClass();a.downloadClass=a.btnDefaultClass()});a.on("mount",function(){setTimeout(function(){a.setMessage("The generation of the sitemap was not started yet.","info")},500)});a.apiserverURL=function(){if(!b.proxyURL){var c="&origin_system="+encodeURIComponent(a.systemName().toLowerCase());
c+="&max_fetchers="+encodeURIComponent(a.maxFetchers());c+="&ignore_embedded_content="+encodeURIComponent(a.ignoreEmbeddedContent());c+="&reference_count_threshold="+encodeURIComponent(a.referenceCountThreshold());c+="&query_params_to_remove="+encodeURIComponent(a.queryParamsToRemove());c+="&disable_cookies="+encodeURIComponent(a.disableCookies());c+="&enable_index_file="+encodeURIComponent(a.enableIndexFile());return b.dev?"http://marco-desktop:9999/sitemap/v2/"+a.websiteURL64()+"?pdfs=1"+c:"https://api.marcobeierer.com/sitemap/v2/"+
a.websiteURL64()+"?pdfs=1"+c}c=b.proxyURL;"Joomla"==a.systemName()&&(c+="&base64url="+a.websiteURL64()+"&identifier="+a.identifier());return c};a.generate=function(c){c.preventDefault();a.downloadDisabled=!0;a.generateDisabled=!0;a.pageCount=0;a.stats=null;a.setMessage("The sitemap is being generated. Please wait a moment.","warning");a.generateClass=a.btnPrimaryClass();a.downloadClass=a.btnDefaultClass();a.retries=0;var e=function(){var c="";a.hasToken()&&(c="BEARER "+a.token());jQuery.ajax({method:"GET",
url:a.apiserverURL(),headers:{Authorization:c,"Cache-Control":"no-store"}}).done(function(c,d,f){a.retries=0;f.getResponseHeader("Content-Type").startsWith("application/xml")||f.getResponseHeader("Content-Type").startsWith("text/xml")?(a.sitemapGeneratorBlob=new Blob([f.responseText],{type:"application/xml"}),a.href=(window.URL||window.webkitURL).createObjectURL(a.sitemapGeneratorBlob),a.downloadDisabled=!1,a.generateDisabled=!1,1==f.getResponseHeader("X-Limit-Reached")?a.setMessage("The Sitemap Generator reached the URL limit and the generated sitemap probably isn't complete. You may buy a token for the <a href=\""+
b.professionalURL+"\">Sitemap Generator Professional</a> to crawl up to 50'000 URLs and create a complete sitemap. Additionally to a higher URL limit, the professional version also adds images and videos to your sitemap.","danger"):(c="Your sitemap was generated successfully. You can download the sitemap now.",["WordPress","Joomla"].includes(a.systemName())&&(c="The generation of the sitemap was successful. The sitemap was saved as sitemap.xml in the "+a.systemName()+" root folder. Please see the stats below."),
a.setMessage(c,"success")),null!=f.getResponseHeader("X-Stats")&&(a.stats=JSON.parse(f.getResponseHeader("X-Stats"))),a.generateClass=a.btnDefaultClass(),a.downloadClass=a.btnPrimaryClass()):(a.pageCount=c.page_count,0<a.pageCount&&(c="The sitemap is being generated. Please wait a moment.<br />"+a.pageCount+" URLs already processed.",a.setMessage(c,"warning")),setTimeout(e,1E3));a.update()}).fail(function(b,c,d){c=b.status;a.generateDisabled=!1;if(401==c)a.setMessage("The validation of your token failed. The token is invalid or has expired. Please try it again or contact me if the token should be valid.",
"danger");else if(500==c)1==b.getResponseHeader("X-Write-Error")?a.setMessage("Could not write sitemap to file. The reason for that could be a permission issue or that not enough space is available.","danger"):![void 0,null,""].includes(b.responseJSON)&&b.getResponseHeader("Content-Type").startsWith("application/json")?a.setMessage("The generation of your sitemap failed with the error:<br/><strong>"+b.responseJSON+"</strong>.","danger"):a.setMessage("The generation of your sitemap failed. Please try it again.",
"danger");else if(503==c)a.setMessage("The backend server is temporarily unavailable. Please try it again later.");else if(504==c&&1==b.getResponseHeader("X-CURL-Error"))b=b.responseJSON,""==b?a.setMessage("A cURL error occurred. Please contact the developer of the extensions.","danger"):a.setMessage("A cURL error occurred with the error message:<br/><strong>"+b+"</strong>.","danger");else{if(0==c&&3>a.retries){a.retries++;setTimeout(e,1E3);a.update();return}a.setMessage("The generation of your sitemap failed. Please try it again or contact the developer of the extensions.",
"danger")}a.update()})};e()};a.download=function(b){window.navigator.msSaveOrOpenBlob&&a.sitemapGeneratorBlob&&window.navigator.msSaveOrOpenBlob(a.sitemapGeneratorBlob,"sitemap.xml")};a.bool2text=function(a){return a?"Yes":"No"}});
