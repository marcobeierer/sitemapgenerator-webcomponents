...
$jscomp.polyfill("Array.prototype.includes",function(b){return b?b:function(a,b){var c=this;c instanceof String&&(c=String(c));var d=c.length;b=b||0;for(0>b&&(b=Math.max(b+d,0));b<d;b++){var f=c[b];if(f===a||Object.is(f,a))return!0}return!1}},"es7","es3");
...
$jscomp.polyfill("String.prototype.startsWith",function(b){return b?b:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var d=c.length,f=a.length;b=Math.max(0,Math.min(b|0,c.length));for(var h=0;h<f&&b<d;)if(c[b++]!=a[h++])return!1;return h>=f}},"es6","es3");
...
url:a.apiserverURL(),headers:{Authorization:c,"Cache-Control":"no-store"}}).done(function(c,d,g){a.retries=0;g.getResponseHeader("Content-Type").startsWith("application/xml")?(a.sitemapGeneratorBlob=new Blob([c],{type:"application/xml"}),a.href=(window.URL||window.webkitURL).createObjectURL(a.sitemapGeneratorBlob),a.downloadDisabled=!1,a.generateDisabled=!1,1==g.getResponseHeader("X-Limit-Reached")?a.setMessage("The Sitemap Generator reached the URL limit and the generated sitemap probably isn't complete. You may buy a token for the <a href=\""+
...
a.setMessage(c,"success")),null!=g.getResponseHeader("X-Stats")&&(a.stats=JSON.parse(g.getResponseHeader("X-Stats"))),a.generateClass=a.btnDefaultClass(),a.downloadClass=a.btnPrimaryClass()):(a.pageCount=c.page_count,0<a.pageCount&&(c="The sitemap is being generated. Please wait a moment.<br />"+a.pageCount+" URLs already processed.",a.setMessage(c,"warning")),setTimeout(e,1E3));a.update()}).fail(function(b,c,d){c=b.status;a.generateDisabled=!1;if(401==c)a.setMessage("The validation of your token failed. The token is invalid or has expired. Please try it again or contact me if the token should be valid.",
